...
extract_gusigun_from_sido <- function(sido_name = "^서울") {
  
  moving_2021_seoul <- moving_2021_lvl_04 %>% 
    filter(str_detect(전출지, sido_name) ) %>% 
    mutate(전입지 = ifelse(str_detect(전입지, sido_name), 전입지, str_sub(전입지, 1, 2))) %>% 
    group_by(전출지, 전입지) %>% 
    summarise(이사인구수 = sum(계)) %>% 
    arrange(desc(이사인구수)) %>% 
    ungroup() %>% 
    mutate(전출지 = str_remove(전출지, sido_name) %>% str_trim(.),
           전입지 = str_remove(전입지, sido_name) %>% str_trim(.))
...
extract_gusigun_from_sido("^강원")



# 6.2. 네트워크 데이터 ----------------------------------------------------------------
## 6.2.1. 네트워크 데이터 정제: 결점 -----
### 시도 인구

make_network_data <- function(sido_name = "^강원") {
  ...
  sido_tbl <- extract_gusigun_from_sido(sido_name)
  
  ### 특정시도 구시군 인구
  sido_gusigun <- sido_tbl %>% 
    count(전출지) %>% 
    pull(전출지) %>% 
    dput()
  
  pop_2021_sido <- pop_2021_raw %>% 
    filter( 행정구역명 %in% sido_gusigun) %>% 
    filter(성별 == "전체") %>% 
    slice(1:25) %>% 
    select(시도 = 행정구역명, 인구수=합계)
  
  moving_sido_node <- bind_rows(pop_2021, pop_2021_sido) %>% 
    filter(! str_detect(시도, str_extract(sido_name, "[가-힣].*")) ) %>% 
    mutate(인구수 = 인구수/10^4)
  
  
  ## 5.2.2. 네트워크 데이터 정제: 연결선 -----
  
  moving_sido_edge <- sido_tbl %>%
    rename(from = 전출지,
           to   = 전입지) %>% 
    mutate(이사인구수 = 이사인구수/10^4)
  ...
  ## 5.2.3. 네트워크 데이터 변환(tidygraph) -----
  moving_seoul_nw <- tbl_graph(
    nodes = moving_sido_node, edges = moving_sido_edge, directed = TRUE
  )
  
  return(moving_seoul_nw)
}

make_network_data("^서울") 

str_extract("^경기", "[가-힣].*")


extract_gusigun_from_sido("^강원") %>% View
